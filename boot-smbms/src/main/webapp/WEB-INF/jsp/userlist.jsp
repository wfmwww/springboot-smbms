...
<%@include file="/WEB-INF/jsp/common/head.jsp"%>
        <div class="right">
			...
			<div>
				<input type="button" value="导出用户信息" id="exportAll">
				<input type="button" value="导出模板" id="exportModel">
				<form action="${pageContext.request.contextPath }/sys/poi/uploadUserExcel.html" method="post"
				enctype="multipart/form-data">
					<input type="file" name="myFile" id="myFile">
					<input type="submit" name="上传" id="uploadUserExcel">
				</form>
			</div>
            <!--用户-->
            <table class="providerTable" cellpadding="0" cellspacing="0">
                <tr class="firstTr">
                   ...
                   <c:forEach var="user" items="${userList}" varStatus="status">
					   <tr <c:if test="${user.id == userSession.id}">style="color: red" </c:if>>
						<td>
						...
						<span><a class="viewUser" href="javascript:;" userid=${user.id } username=${user.userName }><img src="${pageContext.request.contextPath }/statics/images/read.png" alt="查看" title="查看"/></a></span>
							<c:if test="${userSession.userRole!=3 ||user.id == userSession.id}">
						<span><a class="modifyUser" href="javascript:;" userid=${user.id } username=${user.userName }><img src="${pageContext.request.contextPath }/statics/images/xiugai.png" alt="修改" title="修改"/></a></span>
							...
							<c:if test="${userSession.userRole!=3}">
							<c:if test="${user.id != userSession.id}">
			...
			<c:if test="${userList.size()==0}"><h1 align="center" style="color: red">未查询到</h1></c:if>
			...
			<div class="providerAdd" id="viewById" style="display: none">
				<div>
					<label>用户编码：</label>
...
<div class="remove" id="removeUse">
    <div class="removerChid">
        ...
        <div class="removeMain">
            <p>你确定要删除该用户吗？</p>
            <a href="javascript:cancleBtn()" id="yes">确定</a>
            <a href="#" id="no">取消</a>
        </div>
    </div>
</div>
