...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/sys/bill/ajaxPro.html",//请求的url
	...
	billCode.on("blur",function(){
		if(billCode.val() != null && billCode.val() != ""){
			$.get(path+"/sys/bill/ajaxBillCode.html","billCode="+billCode.val(),function(data){
				var ajaxobj = $.parseJSON(data);
				if(ajaxobj=="true") {
					validateTip(billCode.next(), {"color": "red"}, imgNo + " 编码已经存在，请重新输入", false);
				}
				else {
	...
	productCount.on("focus",function(){
		validateTip(productCount.next(),{"color":"#666666"},"* 请输入大于0的正自然数",false);
	}).on("blur",function(){
		if(productCount.val() != null && productCount.val() != ""){
			var reg = /^\+?[1-9]\d*$/;
			//alert(reg.test(productCount.val()));
			if (!reg.test(productCount.val())) {
				validateTip(productCount.next(), {"color": "red"}, imgNo + " 请输入大于零的正整数", false);
			} else {
				validateTip(productCount.next(),{"color":"green"},imgYes,true);
			}
		}else{
			validateTip(productCount.next(),{"color":"red"},imgNo+" 数量不可以为空",false);
		}
	});
	totalPrice.on("focus",function(){
		...
		if(totalPrice.val() != null && totalPrice.val() != ""){
			var reg = /[1-9]{1,10}(\.\d{0,10})?$/;
			//alert(reg.test(totalPrice.val()));
			if (!reg.test(totalPrice.val())) {
				validateTip(totalPrice.next(),{"color":"red"},imgNo+" 请输入规范的价格",false);
			}else {
				validateTip(totalPrice.next(),{"color":"green"},imgYes,true);
			}
		}else{
			validateTip(totalPrice.next(),{"color":"red"},imgNo+" 价格不可以为空",false);
		}
	});
	...
	/*productCount.on("focus",function(){
		...
		this.value = priceReg1(this.value);
	}).on("blur",function(){
		this.value = priceReg1(this.value);
	});
