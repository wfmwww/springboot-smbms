...
$(function(){
	...
	$(".viewUser").on("click",function(){
		...
		var obj = $(this);
		$("#viewById").css("display","block");
		//window.location.href=path+"/user/view/"+ obj.attr("userid");
		$.ajax({
			type: "GET",
			url: path + "/sys/user/userView.html",
	...
	$(".modifyUser").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/user/upUser.html?uid="+ obj.attr("userid");
	});
...
/*	$(".deleteUser").on("click",function(){
	...
	$("#exportAll").click(function () {
		location.href = path+"/sys/poi/downloadUserAll.html";
	});
//下载模板
	$("#exportModel").click(function () {
		location.href = path+"/sys/poi/downloadUserModel.html";
	});

	//上传
		$("#uploadUserExcel").click(function () {
			//获取名称
			...
			if(result==null || result==""){
				...
				var tail = result.substring(point+1);
	...
	$(".deleteUser").on("click",function(){
		...
		if(confirm("你确定要删除用户【"+obj.attr("username")+"】吗？")){
			$.ajax({
				type:"GET",
				url:path+"/sys/user/delUser.html",
				data:{uid:obj.attr("userid")},
				dataType:"json",
				success:function(data){
					var ajaxobj = $.parseJSON(data);
					if(ajaxobj.delResult == "true"){//删除成功：移除删除行
						//alert("删除成功");
						//obj.parents("tr").remove();
						window.location.href=path+"/sys/user/userList.html/";
					}else if(ajaxobj.delResult == "false"){//删除失败
						alert("对不起，删除用户【"+obj.attr("username")+"】失败");
					}else if(ajaxobj.delResult == "notexist"){
						alert("对不起，用户【"+obj.attr("username")+"】不存在");
					}
				},
				error:function(data){
